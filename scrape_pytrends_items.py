        ...
        for i, item in enumerate(items_to_process, 1):
            ...
            try:
                ...
                if (
                    ...
                    and len(article_data["content"]) > 200
                ):
                    ...
                    if article_data.get("published_date"):
                        if not item.source_metadata:
                            item.source_metadata = {}
                        item.source_metadata["published_date"] = article_data[
                            "published_date"
                        ]

                    print(
                        f"  ✅ Scraped {len(article_data['content'])} chars of content"
                    )
                    scraped_count += 1
                else:
                ...
                if "rate" in error_msg or "limit" in error_msg or "429" in error_msg:
                    ...
                    print(
                        f"Run this script again later to continue scraping remaining items"
                    )
                    failed_count += 1
                    break
                else:
