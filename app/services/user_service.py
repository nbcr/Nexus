...
async def update_user_profile(
    db: AsyncSession, user_id: int, profile: UserProfile
) -> User:
...
async def get_user_preferences(db: AsyncSession, user_id: int) -> UserPreferences:
    ...
    if not profile:
        return UserPreferences(
            interests=[], preferred_categories=[], reading_preferences={}
        )
...
async def get_user_stats(db: AsyncSession, user_id: int, timeframe: str) -> UserStats:
    ...
    interactions_query = select(
        ...
        func.count()
        .filter(UserInteraction.interaction_type == "follow")
        .label("follows"),
    ).where(
        UserInteraction.user_id == user_id, UserInteraction.created_at >= start_time
    )
