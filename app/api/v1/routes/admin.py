...
async def get_tracking_log(
    ...
    result = await db.execute(
        ...
        .where(UserInteraction.interaction_type.in_(['interest_high', 'interest_medium', 'interest_low']))
...
async def get_user_details(
    ...
    avg_duration_result = await db.execute(
        select(func.avg(UserInteraction.duration_seconds))
        .where(UserInteraction.user_id == user_id)
    )
...
async def get_analytics(
    ...
    avg_duration_result = await db.execute(
        select(func.avg(UserInteraction.duration_seconds))
        .where(and_(
            UserInteraction.interaction_type.in_(['interest_high', 'interest_medium', 'interest_low']),
            UserInteraction.created_at >= start_date,
            UserInteraction.created_at < end_date
        ))
    )
