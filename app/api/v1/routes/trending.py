...
async def refresh_trending_topics(
    ...
    try:
        if not force:
            background_tasks.add_task(trending_service.save_trends_to_database, db)
            return []
...
async def get_current_trends(
    ...
    """
    try:
        query = select(Topic).where(Topic.trend_score >= min_score)

        if category:
            query = query.where(Topic.category == category)

        query = query.order_by(Topic.trend_score.desc(), Topic.created_at.desc()).limit(
            limit
        )
...
async def get_trending_content(
    ...
    try:
        ...
        query = query.order_by(
            Topic.trend_score.desc(), ContentItem.created_at.desc()
        ).limit(limit)
...
async def get_enhanced_trends(
    ...
    include_historical: bool = Query(
        False, description="Include historical trend data"
    ),
    timeframe: str = Query(
        "24h", regex="^(24h|7d|30d)$", description="Timeframe for analytics"
    ),
    db: AsyncSession = Depends(get_db),
    ...
    try:
        ...
        result = await db.execute(
            query.order_by(Topic.trend_score.desc()).offset(offset).limit(page_size)
        )
        ...
        for topic in topics:
            enhanced = await trending_service.enhance_topic(
                db, topic, include_historical=include_historical, timeframe=timeframe
            )
