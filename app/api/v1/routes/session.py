...
def get_session_token(
    request: Request, nexus_session: Optional[str] = Cookie(default=None)
) -> str:
    """
...
async def track_content_interest(
    ...
    try:
        ...
        interaction_type = (
            "interest_high"
            if interest_data.interest_score >= 70
            else (
                "interest_medium"
                if interest_data.interest_score >= 50
                else "interest_low"
            )
        )
        ...
        await track_content_interaction(
            ...
            duration_seconds=interest_data.hover_duration_ms
        ...
        raise HTTPException(
            status_code=500, detail=f"Interest tracking failed: {str(e)}"
        )
...
async def update_view_duration(
    ...
    try:
        await update_interaction_duration(
            db, session_token, content_id, duration_data.duration_seconds
        )

        return {
            "status": "updated",
            "duration_seconds": str(duration_data.duration_seconds),
        }
    except Exception as e:
...
async def get_user_history(
    ...
    try:
        ...
        return history, {"is_anonymous": current_user is None}
...
async def migrate_session_to_user_account(
    request: Request, user_id: int = Path(..., ge=1), db: AsyncSession = Depends(get_db)
) -> Dict[str, Any]:
    """
