    ...
    responses={
        200: {
            ...
            "content": {
                "application/json": {
                    "example": [
                        {"id": 1, "title": "AI in Healthcare", "trend_score": 0.85}
                    ]
                }
            },
        }
...
async def get_topics(
    ...
    limit: int = Query(
        20, ge=1, le=100, description="Maximum number of topics to return"
    ),
    category: Optional[str] = Query(None, description="Filter topics by category"),
    min_trend_score: Optional[float] = Query(
        None, ge=0, le=1, description="Minimum trend score"
    ),
...
async def get_topic(
    ...
    include_unpublished: bool = Query(
        False, description="Include unpublished content items"
    ),
    db: AsyncSession = Depends(get_db),
) -> TopicWithContent:
    """
    ...
    if not include_unpublished:
        topic.content_items = [
            item for item in topic.content_items if item.is_published
        ]
...
async def create_topic(
    topic_data: TopicCreate, db: AsyncSession = Depends(get_db)
) -> TopicSchema:
    """
    Create a new topic.
    ...
    existing = await db.execute(
        select(Topic).where(
            Topic.normalized_title == topic_data.title.lower().replace(" ", "_")
        )
    )
