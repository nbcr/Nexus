...
async def refresh_trending_topics(db: AsyncSession = Depends(get_db)):
...
async def get_current_trends(db: AsyncSession = Depends(get_db), limit: int = 20):
...
async def get_trending_content(db: AsyncSession = Depends(get_db), limit: int = 10):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch trending content: {str(e)}"
        )
...
async def get_enhanced_trends(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch enhanced trends: {str(e)}"
        )
