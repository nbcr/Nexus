...
## 📋 KEPT FOR REFERENCE (Old Completed Items)
...
- ✅ Backend checks token validity in `/login` route
- ✅ Redirects to `/` if valid token exists
- ✅ Clears invalid tokens
- ⚠️ Need to verify this works after templating changes
...
- ⚠️ Admin check exists in old index.html JavaScript
- ⚠️ Needs to be added to base.html template
- ⚠️ Conditional rendering based on user.is_admin
...
- ✅ Vertical alignment fixed with form-row-inline
...
- ⚠️ Need to check if X button is visible/functional
- ⚠️ Check if back button closes modal
- ⚠️ Check if ESC key closes modal
