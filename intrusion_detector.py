...
class IntrusionDetector:
    ...
    def analyze_log_line(self, line):
        ...
        if attack_type:
            severity = self.assess_severity(attack_type, url)
            self.log_attack(ip, url, user_agent, attack_type, severity)
    ...
    def assess_severity(self, attack_type, url):
    ...
    def block_cloudflare(self, ip):
        ...
        api_key = self.config.get('cloudflare_api_key') or os.getenv('CLOUDFLARE_API_KEY')
        zone_id = self.config.get('cloudflare_zone_id') or os.getenv('CLOUDFLARE_ZONE_ID')
        
        if not api_key or not zone_id:
            self.logger.warning(f"[CLOUDFLARE] Missing API credentials, cannot block {ip}")
            return
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                ''', (rule_id, ip))
                self.conn.commit()
                self.logger.info(f"[CLOUDFLARE] IP {ip} blocked successfully (Rule ID: {rule_id})")
            else:
                self.logger.error(f"[CLOUDFLARE] Failed to block {ip}: {response.status_code} - {response.text}")
    ...
    def unblock_cloudflare(self, ip, rule_id):
        ...
        if not api_key or not zone_id:
            self.logger.warning(f"[CLOUDFLARE] Missing API credentials, cannot unblock {ip}")
            return
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"[CLOUDFLARE] Failed to unblock {ip}: {response.status_code}")
        except Exception as e:
        ...
        while True:
            try:
                ...
                for line in new_lines:
                    ...
                    if result:
                        self.logger.warning(f"[ALERT] {result['attack_type']} from {result['ip']} - {result['url']}")
