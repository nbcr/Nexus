    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                return {"suggested_category": suggested, "model": "groq/llama-3.3-70b"}
...
async def check_recent_categorization(hours: int = 24, limit: int = 20):
    ...
    hourly_count = counts["hourly"].get(current_hour, 0)
    print(
        f"📊 Rate limit status: {daily_count}/{DAILY_LIMIT} daily, {hourly_count}/{HOURLY_LIMIT} hourly\n"
    )
    ...
    async with AsyncSessionLocal() as db:
        ...
        for id, title, category, description, created_at in items:
            ...
            if GROQ_API_KEY:
                ...
                print(
                    "❌ No API key configured. Set GROQ_API_KEY or TOGETHER_API_KEY environment variable."
                )
                print("\nGet free API keys:")
        ...
        print(
            f"   Accuracy: {((len(items) - len(mismatches)) / len(items) * 100):.1f}%"
        )
        ...
        if mismatches:
            ...
            for item in mismatches:
                print(
                    f"   UPDATE topics SET category = '{item['suggested']}' WHERE id = (SELECT topic_id FROM content_items WHERE id = {item['id']});"
                )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Check content categorization quality using AI"
    )
    parser.add_argument(
        "--hours", type=int, default=24, help="Check content from last N hours"
    )
    parser.add_argument(
        "--limit", type=int, default=20, help="Max number of items to check"
    )
