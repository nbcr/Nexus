...
async def test_pytrends():
    ...
    if trending:
        ...
        print("-" * 60)
        related_topics = await pytrends_service.fetch_related_topics(
        ...
        print("-" * 60)
        related_queries = await pytrends_service.fetch_related_queries(
            keyword, timeframe="now 1-d"
        )
        ...
        print("-" * 60)
        interest = await pytrends_service.fetch_interest_over_time(
            [keyword], timeframe="now 7-d"
        )
